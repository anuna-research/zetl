...
`Document` (per `tools/zetl-ast-schema-v1.json` v1.0) that must round-trip
**bit-for-bit** through every first-party helper library's identity
transform:

- **Rust** — `serde_json::from_value::<Document>(_)` + `to_value(_)`
- **Python** — `zetl_ast.run(lambda ast, ctx: ast)` (persistent mode)
- **JavaScript** — `zetl-ast-js` `run((ast) => ast)` (persistent mode)
...
2. Ensure it is canonical: objects use insertion-order keys matching
   what the Rust helper emits (run through `zetl ast sample` if in
