//! Integration tests for `ztl cap genkey` (SPEC-034 REQ-3419, TEST-3419).
//!
//! Covers:
//!
//! * The command exits 0 and prints BOTH env-var assignments to stdout.
//! * The printed `ztl_CAP_SECRET` round-trips through the pure-core
...
//!   the remediation text ("re-run `ztl cap genkey`") is surfaced.
...
fn run_genkey(extra_args: &[&str]) -> (std::process::Output, String, String) {
    ...
    let assert = cargo_bin_cmd!("ztl").args(&args).assert().success();
...
fn genkey_prints_both_env_vars_and_succeeds() {
    ...
    assert!(
        stdout.contains("export ztl_CAP_SECRET="),
        "missing ztl_CAP_SECRET export line:\n{stdout}",
    );
    assert!(
        stdout.contains("export ztl_CAP_SIGNING_KEY="),
        "missing ztl_CAP_SIGNING_KEY export line:\n{stdout}",
    );
}
...
fn genkey_output_parses_through_pure_core() {
    let (_out, stdout, _stderr) = run_genkey(&[]);
    let secret_b64 = extract_export(&stdout, "ztl_CAP_SECRET");
    // Must round-trip the checksum check that the build uses at startup.
    let parsed = ztl::cap::genkey::decode_secret(&secret_b64)
        .expect("genkey output must parse under pure-core decoder");
    assert_eq!(parsed.version(), ztl::cap::genkey::SECRET_VERSION_V1);
    assert_eq!(
        parsed.random_body().len(),
        ztl::cap::genkey::SECRET_RANDOM_LEN
    );

    // Signing key must be 32 base64-decoded bytes.
    let sk_b64 = extract_export(&stdout, "ztl_CAP_SIGNING_KEY");
    let sk = STANDARD.decode(&sk_b64).expect("signing key is base64");
    assert_eq!(sk.len(), ztl::cap::genkey::SIGNING_KEY_LEN);
}
...
fn tampered_checksum_yields_remediation_error() {
    ...
    let err = match ztl::cap::genkey::decode_secret(&mangled) {
        Ok(_) => panic!("tampered checksum must be rejected"),
        Err(e) => e,
    };
    assert!(
        matches!(err, ztl::cap::genkey::SecretParseError::Checksum { .. }),
        "expected Checksum variant, got {err:?}",
    );
    let rendered = format!("{err}");
    assert!(
        rendered.to_lowercase().contains("re-run `ztl cap genkey`"),
...
fn genkey_never_logs_secret_to_stderr() {
    ...
    let secret_b64 = extract_export(&stdout, "ztl_CAP_SECRET");
    let signing_b64 = extract_export(&stdout, "ztl_CAP_SIGNING_KEY");
    assert!(
        !stderr.contains(&secret_b64),
        "ztl_CAP_SECRET leaked to stderr:\n{stderr}",
    );
    assert!(
        !stderr.contains(&signing_b64),
        "ztl_CAP_SIGNING_KEY leaked to stderr:\n{stderr}",
    );
}
...
fn genkey_json_mode_is_parseable() {
    ...
    assert_eq!(j["secret"]["env"], "ztl_CAP_SECRET");
    assert_eq!(j["signing_key"]["env"], "ztl_CAP_SIGNING_KEY");
    // Round-trip the embedded secret through the pure-core decoder.
    let secret_b64 = j["secret"]["value"]
        .as_str()
        .expect("secret.value is a string");
    ztl::cap::genkey::decode_secret(secret_b64).expect("emitted secret parses");
}
...
fn two_invocations_produce_distinct_secrets() {
    ...
    let a_secret = extract_export(&a_stdout, "ztl_CAP_SECRET");
    let b_secret = extract_export(&b_stdout, "ztl_CAP_SECRET");
    assert_ne!(a_secret, b_secret);
    let a_sk = extract_export(&a_stdout, "ztl_CAP_SIGNING_KEY");
    let b_sk = extract_export(&b_stdout, "ztl_CAP_SIGNING_KEY");
    assert_ne!(a_sk, b_sk);
}
