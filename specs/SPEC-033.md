...
SPEC-032 established a three-stage render-hook system (`pre-parse` / `transform` / `post-render`) with first-class file selection, a versioned AST JSON schema (`ztl-ext`), and a reserved `ast_type` manifest field as forward-compatibility machinery. What SPEC-032 deliberately left for a successor is the strategic question of **what plugin ecosystems ztl actually integrates with**. This spec answers that question.

ztl's goal is to **utilise existing plugin ecosystems, not to build its own**. Writing our own Callouts, Tasks, Admonition extensions as first-party Python hooks (the plan in SPEC-032 §§ REQ-3212, ADR-3204) duplicates work that mature ecosystems already solve better. The empirical case for this is strong: a 2026-04-19 scan of seven plugin ecosystems (cached at `tools/ecosystem-scan.md`) found that (a) Pandoc filters alone cover 63% of PKM-relevant categories (citations, crossrefs, transclusion, tables, math) in their top-30, (b) mdBook preprocessors have the best maintenance signal of any ecosystem (90% active in top-30) and architecturally match SPEC-032's JSON-stdio protocol exactly, and (c) remark is the largest JS ecosystem (539 plugins) with deep coverage of the web-docs use case. Writing our own extensions against those ecosystems' gaps is far cheaper than rebuilding their functional core.

This spec introduces an **ecosystem adapter** as a first-class concept: a declarative bridge between ztl's hook runtime and one of several supported plugin ecosystems, each with its own AST format, invocation conventions, and dependency model. v1 ships three adapters (Pandoc, mdBook, remark); the architecture accommodates additional ecosystems (djot, Quarto, MkDocs, and others) as incremental additions rather than redesigns.
...
ztl translates ztl-ext ↔ the ecosystem's AST at the adapter boundary, emulates the ecosystem's invocation conventions (env vars, argv, protocol shape), dispatches the plugin, and converts the response back. Mixed-ecosystem pipelines (a pandoc filter, followed by an mdBook preprocessor, followed by a remark plugin) compose because ztl owns every translation.
...
- **Leverage beats originality.** A ten-year-old Pandoc filter like `pandoc-crossref` has had more eyeballs, edge-case fixes, and production deployments than anything ztl could ship as a v1 canonical extension. The right strategy is to make `pandoc-crossref` work under `ztl build`, not to reimplement crossref logic.
- **Ecosystem coverage compounds.** Adopting Pandoc gets the academic/technical-writing audience (citations, crossrefs, bibliography). Adopting mdBook gets the docs-site audience (diagrams, math, TOC). Adopting remark gets the JS-tooling audience (MDX, GFM variants, math). Three adapters cover most of what ztl's users would want.
...
- **Users already have installed these.** Pandoc is on most academic/technical writers' machines. Rust developers have `cargo install`'d mdBook preprocessors. Web developers have a `node_modules` full of remark plugins. ztl consuming those existing installations respects the user's context instead of demanding they start over.

### 1.2 Design Principles

1. **ztl owns the translations, not the plugins.** Every `ecosystem ↔ ztl-ext` translator is a ztl-maintained Rust module with fuzz-quality round-trip tests. We do not delegate translation fidelity to third-party adapter binaries or plugins themselves.
2. **Marker-convention discipline for lossy boundaries.** ztl concepts that don't map natively to a foreign AST (wikilinks, embeds, SPL blocks) are represented via that ecosystem's extension mechanism (Pandoc `Span` with classes, mdast custom nodes, mdBook literal marker syntax). A plugin that strips markers breaks round-trip; ztl detects and warns.
3. **Emulate the ecosystem's invocation contract faithfully.** A Pandoc filter run under ztl sees `PANDOC_VERSION`, `PANDOC_API_VERSION`, argv[1] = output format, and pandoc-types on stdin. An mdBook preprocessor sees the `supports <renderer>` probe and `{context, book}` on stdin. A remark plugin is loaded via `require()` in a Node harness. Ecosystem-native plugins behave as they would in their native host.
4. **Optional-runtime model.** Each ecosystem has an optional runtime dep (Pandoc binary; Node + npm; `cargo install`'d preprocessors). ztl detects at startup, reports what's available, disables adapters with missing deps, and provides actionable installation hints. ztl does not bundle any ecosystem runtime.
5. **One internal AST, many external views.** ztl's canonical internal representation stays `ztl-ext` (as in SPEC-032). Ecosystem-specific formats (pandoc-types, mdast, mdBook Book) are views at the hook boundary. The core engine doesn't know about Pandoc or remark.
...
7. **No ecosystem bundling.** ztl is not a Pandoc redistribution, nor an mdBook fork, nor a remark wrapper. Users install the ecosystem's own tooling; ztl orchestrates.
...
- Per-ecosystem translators: `ztl-ext ↔ pandoc-types`, `ztl-ext ↔ mdast`, `ztl-ext ↔ mdBook Book envelope` (the last is more "protocol wrapper" than "AST translator" since mdBook operates on raw Markdown strings).
- A **parser registry** with per-ecosystem parser adapters: Pandoc (via subprocess to installed `pandoc` binary) for pandoc-ext hooks, pulldown-cmark (default) for ztl-ext hooks. Parser selection is per-page (frontmatter or directory-pattern). Adds the foundation for further parser additions (djot, pandoc.wasm) without protocol changes.
- Ecosystem runtime detection at ztl startup; actionable diagnostics when an adapter's runtime is missing.
- Plugin discovery per ecosystem: `.ztl/hooks/transform.d/<name>.toml` manifests with `ecosystem = "pandoc" | "mdbook" | "remark"`; ztl resolves the executable or package per ecosystem conventions.
...
- Compat matrix: `tools/ztl-ecosystem-matrix.toml` recording tested plugin versions per ecosystem at `supported` / `partial` / `experimental` tiers.
...
- `ztl ecosystem check` subcommand — reports which ecosystem runtimes are detected, versions, and the set of adapters available this session.
...
- mdBook as a full build pipeline — ztl does not attempt to produce mdBook-compatible output. It runs mdBook preprocessors *as a convenience*, adapting ztl's vault into an mdBook `Book` envelope for the preprocessor and unpacking the result. No book-level output.
- remark as a parser (replacing pulldown-cmark) — v1 keeps remark as a *hook* runtime only; it does not parse ztl's Markdown. Users who want remark's parsing semantics can run `remark-parse` as a pre-parse-stage hook. Full parser-layer adoption is a v1.1 concern.
- Automatic npm install of remark plugins — ztl requires users to have already installed plugins in a local `node_modules` or globally. No package-management orchestration.
...
- Installation assistance beyond diagnostics — ztl prints actionable hints ("install pandoc: `brew install pandoc`") but does not run the installer.
...
**Role:** Researcher maintaining a ztl vault of papers-in-progress with `[[wikilink]]` cross-references. Uses `pandoc-crossref` for numbered figures/sections, `pandoc-citeproc` for bibliography, `pantable` for complex tables.
**Goal:** Publish the vault as a static site (`ztl build`) with all Pandoc-filter transformations applied, without leaving ztl.
**Constraints:** Pandoc already installed (`brew install pandoc`). Has a `references.bib`. Does not want to learn a new extension API.

**Happy path:**

1. User runs `ztl ecosystem check` → sees `pandoc: detected (3.1.12.1); pandoc-types: 1.23`.
2. Writes `.ztl/hooks/transform.d/crossref.toml`:
   ```toml
   stage = "transform"
   ecosystem = "pandoc"
   exec = "pandoc-crossref"
...
4. `ztl build` runs; on each page, ztl parses with Pandoc (because `transform` stage has `ecosystem="pandoc"` hooks and the page's frontmatter says `parser: pandoc`), emits pandoc-types AST, invokes `pandoc-crossref`, then `pandoc-citeproc`, renders to HTML.
5. Wikilinks in the source are preserved across Pandoc-filter invocations via marker conventions (`Span class="ztl-wikilink"` + attrs); pandoc-citeproc's `[@key]` citations render to Chicago-style footnotes.
6. Diagnostic report: `[ztl] ecosystem pandoc: 3 filters loaded, 247 page invocations, 0 round-trip warnings`.

**Failure modes:**

- User has `ecosystem="pandoc"` hook but no pandoc binary → `ztl ecosystem check` fails with hint: `"install pandoc 2.11+ — brew install pandoc / apt install pandoc"`. Hook disabled; build continues with plain output.
- `pandoc-crossref` version incompatible with pandoc binary version → Pandoc's own error surfaces through stderr; ztl forwards it; plugin disabled.
- A plugin strips wikilink markers silently → ztl detects via round-trip compare, logs warning, user sees `"pandoc-crossref dropped 4 wikilink markers on projects/q2-review.md"` in the coverage report.
...
**Goal:** Use these exact preprocessors under `ztl serve` and `ztl build`.
...
1. `ztl ecosystem check` → `mdbook: detected (preprocessors available: mdbook-mermaid, mdbook-toc, mdbook-katex, mdbook-admonish); mdBook binary not found (only needed for full-book pipeline; preprocessors run independently)`.
2. Writes manifests:
   ```toml
   # .ztl/hooks/transform.d/mermaid.toml
   stage = "pre-parse"
   ecosystem = "mdbook"
   exec = "mdbook-mermaid"
   ```
   (mdBook preprocessors operate on raw Markdown text in chapter content, so they fit ztl's `pre-parse` stage not `transform`.)
3. Builds. ztl wraps each page's Markdown in a synthetic mdBook `Book` envelope (one chapter = one ztl page), invokes the preprocessor, unwraps the result, hands the transformed Markdown to the parser.
...
**Role:** MDX / Astro / Docusaurus user migrating content into ztl. Has 15 remark plugins they rely on (`remark-gfm`, `remark-math`, `remark-directive`, `remark-mdx`, `remark-smartypants`, etc.).
**Goal:** Those plugins should run under `ztl build` without requiring a full JS build pipeline.
**Constraints:** Has Node 18+ installed. Happy to `npm install` plugins into a project-local `node_modules`.

**Happy path:**

1. `ztl ecosystem check` → `remark: detected (node v20.10.0); harness at _static/ztl-remark-harness.mjs; resolvable plugins from ./node_modules`.
...
3. Writes manifests:
   ...
   options = {}
   ```
4. `ztl build` starts a persistent Node subprocess with ztl's embedded remark harness. The harness loads plugins via `require()`, receives mdast JSON per page, applies plugins, returns mdast.
5. ztl translates ztl-ext → mdast at the boundary, invokes the harness, translates back.
...
**Role:** ztl contributor wants to add djot support because John MacFarlane's djot parser has features they want (explicit attr syntax, no HTML-passthrough ambiguity).
...
2. Implements `src/ecosystems/djot/translator.rs` with `ztl_ext_to_djot` / `djot_to_ztl_ext` fns.
3. Adds property-test roundtrip suite; implements parser invocation.
4. Adds `ztl-ecosystem-matrix.toml` entry with test fixtures.
...
- A new translator module with bidirectional conversion to/from ztl-ext.
- A registry entry.
...
- `translate_to_foreign(ast: ztlExt) -> ForeignAST` — serialise ztl-ext into the ecosystem's AST shape.
- `translate_from_foreign(ast: ForeignAST) -> ztlExt` — deserialise back to ztl-ext.
...
The trait SHALL be implemented in the ztl binary, not loaded dynamically. Each adapter is compiled into ztl (feature-flagged per ecosystem to manage binary size).
...
- **Translates** ztl-ext to pandoc-types with marker conventions for
  ztl concepts (REQ-3307).
...
- Source is the raw page Markdown; output is Pandoc's HTML. Runs at
  ...
  ztl's own rendering (the default when `mode = "native"`), which
...
- **Stage**: mdBook preprocessors operate on raw Markdown (chapter `.content`), so the adapter runs them at ztl's `pre-parse` stage, not `transform`.
...
- **Persistent harness**: ztl starts a single Node subprocess on first use, loading the `ztl-remark-harness.mjs` script (bundled with ztl under `_static/`), which imports `unified`, and exposes a JSON-RPC-like protocol on stdin/stdout for loading plugins by name, applying them to mdast documents, and unloading.
...
- **Translate** ztl-ext ↔ mdast with marker conventions for ztl concepts (REQ-3308). mdast is closer to ztl-ext than pandoc-types is (both are CommonMark-derived), so translation loss is lower.
...
- `"commonmark"` (default) — pulldown-cmark with ztl's extensions (wikilinks, embeds, SPL blocks).
- `"pandoc"` — invokes `pandoc --from markdown --to json <page>` via the Pandoc adapter to produce pandoc-types; then converted to ztl-ext for internal pipeline.

Parser selection precedence (lowest-to-highest priority):

1. **ztl default**: `commonmark`.
2. **Vault default**: `.ztl/config.toml` `[parse] default = "pandoc"`.
3. **Directory default**: globbed rules in `.ztl/config.toml` `[[parse.rule]]` — e.g., `pattern = "papers/**" parser = "pandoc"`.
4. **Per-page frontmatter**: `parser: pandoc`.

When a `transform`-stage hook has `ecosystem = "pandoc"`, the page's parser SHOULD be `"pandoc"` for best fidelity; otherwise, ztl translates from commonmark-ztl-ext to pandoc-types at the adapter boundary (lossy for pandoc-specific syntax not recognised by pulldown-cmark, but functional).
...
### REQ-3307: ztl-ext ↔ pandoc-types Translation — Marker Conventions
...
Marker conventions for ztl concepts without native pandoc-types
equivalents:

- **Wikilinks** (ztl's `Wikilink` node) ↔ Pandoc `Span` with class `"ztl-wikilink"` and attrs `{target, alias, heading, block_id}`.
- **Embeds** (ztl's `Embed` node) ↔ Pandoc `Span` with class `"ztl-embed"` and attrs `{target, heading, block_id}`.
- **SPL blocks** — `CodeBlock` with language `"spl"` in both directions.
- **Frontmatter** — ztl's `FrontMatter` node ↔ Pandoc's `Meta` map at the document root.
...
### REQ-3308: ztl-ext ↔ mdast Translation — Marker Conventions
...
pandoc-types. mdast node types ztl doesn't natively represent (e.g.,
`definition`, `linkReference`) are preserved as opaque pass-through nodes
via ztl-ext's forward-compat mechanism (the AST schema accepts unknown
...
### REQ-3310: `ztl ecosystem check` Subcommand

The system SHALL provide `ztl ecosystem check [--json]` that:
...
The system SHALL ship `tools/ztl-ecosystem-matrix.toml` recording, per
...
ztl populates the runtime contract from the matching matrix entry
...
`[ztl] ecosystem <eco>: <plugin> not in matrix; behavioural contract
...
signals an active ztl-side maintainer has committed to keeping the
entry green):
...
At ztl startup, before any hook pipeline construction, the system SHALL:

- Invoke every compiled-in ecosystem adapter's `probe()`.
- Log detection results (`[ztl] ecosystem pandoc: detected v3.1.12.1 (pandoc-types 1.23)`).
- Identify which configured hooks (in `.ztl/hooks/` and theme-bundled hooks) target unavailable ecosystems; disable them with typed `ecosystem_runtime_missing` errors and actionable install hints.
- Continue with all other pipeline work.

Missing runtime SHALL NOT be a hard failure of `ztl build` or `ztl serve` unless `--ecosystem-required=<name>` is passed (CI gate mode).
...
- Classify the observed version relative to a `version_range` field
  ...
  - **Minor drift** (same major, observed minor ≥ tested minor) —
    log `[ztl] ecosystem <eco>: <plugin> v<observed> is newer than
...
The system SHALL, on `ztl build`, scan every page's raw Markdown for
...
both parsers, ztl SHALL emit a warning listing:
...
Under `ztl build --strict-parsers`, the warning becomes a fatal
error (CI gate for mixed-parser vaults).

The pattern set ships as `tools/ztl-parser-ambiguity.toml` and is
...
Activating a single ecosystem adapter (probing, loading persistent subprocess where applicable) SHALL complete in ≤ 200 ms at P95 for each ecosystem, measured from `ztl` invocation to readiness for first page invocation.
...
Each ecosystem feature flag SHALL add ≤ 2 MiB to ztl's release binary size. Features are independently toggleable; `cargo build --no-default-features --features="ecosystem-pandoc"` SHALL compile and produce a working ztl with only Pandoc ecosystem support.
...
```
∀ A ∈ ztl-ext:  canonicalise(foreign_to_ztl(ztl_to_foreign(A))) == canonicalise(A)
```

where `canonicalise(·)` is a ztl-owned normaliser that collapses
...
diverse ztl-ext ASTs per release; each must satisfy the canonical-
form equivalence. The generator itself is published at
`tools/ztl-ext-generator/` and pinned to the AST schema major.

**Semantic fidelity** (stronger statement): for the same 10,000-AST
corpus, rendering `A` and `foreign_to_ztl(ztl_to_foreign(A))` to
HTML via ztl's default renderer SHALL produce byte-identical HTML.
...
ztl's adapter and translation layers SHALL introduce no
non-determinism: the same (vault, ztl version, ecosystem matrix
snapshot) SHALL produce byte-identical HTML across runs and
platforms on the adapter side.

**Plugin-internal non-determinism is outside ztl's scope.** Known
sources that ztl does not and cannot control:
...
- remark Node harness: one harness per ztl process; respawn on OOM; ≤ 256 MiB default ceiling.
...
supported), total `ztl build` wall time SHALL increase by **≤ 3×**
relative to a `--no-ecosystems` baseline.

Honest ceiling, not aspirational: three serialised subprocess
pipelines per page, one Node harness round-trip, plus the ztl-ext
...
pub trait EcosystemAdapter: Send + Sync {
    ...
    fn translate_from_foreign(
        &self,
        ast: &ForeignAst,
    ) -> Result<ztlExtDocument, TranslationError>;
...
Identical to Pandoc's own filter contract ([filters.html](https://pandoc.org/filters.html)); filters cannot distinguish ztl-run from pandoc-run invocations.
...
harness → ztl:  {"id": 1, "type": "load_result", "ok": true, "plugin_id": "rp_001"}

ztl → harness:  {"id": 2, "type": "apply", "plugin_id": "rp_001", "ast": {...mdast...}}
...
Harness source (embedded at `_static/ztl-remark-harness.mjs`) is published with ztl; users may supply their own if they need custom plugin resolution or caching.
...
for page:
    ...
    ztl_ext = parser.translate_to_ztl_ext(ast)             # canonical form
```

The ztl-ext AST is always the internal form during hook execution, regardless of parser. Ecosystem-specific AST formats appear only at the adapter boundary.
...
Full node-type mapping tables live at `docs/ecosystems/pandoc-translation.md` and `docs/ecosystems/mdast-translation.md`. Each records the ztl-ext node type, the foreign type, the transformation rule, and a round-trip-fidelity note. Generated from the translator source at CI time so they stay in sync.
...
declares `scope = "vault"`, ztl constructs a synthetic `Book` with
...
- Per-page `build_data` writes (SPEC-032 REQ-3219) are not available
  ...
  writing against.
- Output from a vault-scope hook is spliced back into ztl's per-page
...
pub enum EcosystemSpecific {
    ...
    #[serde(rename = "ztl-native")] ztlNative,
}
```
...
### ADR-3301: ztl-ext Stays the Canonical Internal AST

**Context:** Given Pandoc's #1 position in the ecosystem ranking (19/30 PKM-relevant plugins), an obvious move would be to adopt pandoc-types as ztl's canonical internal AST, making Pandoc filters native and skipping one translation layer. Was considered and debated extensively during SPEC-032 ADR-3201 and in the SPEC-033 drafting conversation.

**Decision:** ztl-ext remains the canonical internal AST. Pandoc-types, mdast, and mdBook's envelope are **views at adapter boundaries**, not internal representations.

**Rationale:**

- **Schema control.** Pandoc's release cadence bumps pandoc-types; adopting it couples ztl to Pandoc's evolution. mdast's versioning is remark's business, not ours. ztl-ext under our control lets us add wikilinks/embeds/SPL as first-class nodes instead of stringly-typed conventions.
...
- **CommonMark simplicity.** ztl-ext is a thin CommonMark superset. pandoc-types has a larger surface (multiple table representations, inline-format intermediaries) because Pandoc supports many output formats (EPUB, LaTeX, DOCX) ztl doesn't care about. Using pandoc-types internally imports complexity we don't benefit from.
...
**Context:** Four mechanisms considered for integrating Pandoc into ztl: (a) binary subprocess, (b) the `pandoc-types` Rust crate + pulldown-cmark (build pandoc-types AST from ztl's own parser), (c) `pandoc.wasm` embedded via wasmtime, (d) direct FFI to Pandoc via Haskell foreign-export-ccall.

**Decision:** (a) binary subprocess. User's installed `pandoc` binary is invoked as a subprocess. ztl does not bundle or embed Pandoc.

**Rationale:**

- **Zero binary bloat.** Alternatives (c) and (d) add 40–150 MiB to ztl's release artifact. Subprocess keeps ztl binary small.
...
- **User trust.** Users install Pandoc themselves via `brew install pandoc` / `apt install pandoc`; ztl doesn't ship someone else's binary. Security and supply-chain clean.
- **Upgrade path.** User upgrades Pandoc independently of ztl; ztl's adapter only needs to keep compatible with pandoc-types API version ranges declared in the matrix.
...
- **User dependency management.** Users need to install Pandoc separately. Mitigated by `ztl ecosystem check` with actionable hints.
...
**Context:** mdBook preprocessors operate on raw Markdown chapter content (`.content: String` field of `Chapter`), not on an AST. Their natural ztl stage is therefore `pre-parse`, not `transform`.
...
"pre-parse caveat" documents the same risk surface for ztl-native
...
**Context:** remark plugins are in-process ESM JavaScript. To run them without embedding a JS runtime in ztl (SPEC-031 tried this with QuickJS and it was rejected on scan data), we need an out-of-process bridge.

**Decision:** Persistent Node subprocess running a ztl-provided harness (`ztl-remark-harness.mjs`) that loads plugins via dynamic `import()` and exposes a JSON-RPC-like protocol on stdin/stdout.

**Rationale:**

- **Node is widely installed.** Any user running remark plugins already has Node 18+.
- **Plugin resolution is Node's native strength.** `node_modules` resolution, ESM imports, and semver ranges are all Node's problem, not ztl's. The harness uses them directly.
...
- **Harness maintenance.** ztl owns the harness script; breaking changes to `unified` or `remark-core` may require harness updates. Harness version pinned in `tools/ztl-ecosystem-matrix.toml`.
...
- **Pandoc**: #1 by PKM-relevance (63% hit rate). The only ecosystem with citations, crossrefs, transclusion at top-30 popularity. Audience: ztl's academic/technical users.
...
For each adapter, run a parameterised conformance suite that exercises probe → translate_to_foreign → invoke_plugin (identity) → translate_from_foreign on a corpus of ztl-ext fixtures. Assert output equals input (identity round-trip).
...
For each matrix plugin at tier `supported`: run `ztl build` on a golden fixture vault with the plugin active; assert output HTML equals the recorded baseline (run under real Pandoc, not ztl, for the baseline) to the normalisation threshold.
...
- Generate 10,000 diverse ztl-ext ASTs via property-based generator.
...
Run ztl with no pandoc installed; assert `[ztl] ecosystem pandoc: missing (install hint: brew install pandoc)` on stderr. Run with `--ecosystem-required=pandoc`; assert exit non-zero.
...
At ztl startup: one log line per registered ecosystem.
`[ztl] ecosystem pandoc: detected v3.1.12.1 (pandoc-types 1.23)`
`[ztl] ecosystem mdbook: detected (preprocessors: mdbook-mermaid, mdbook-toc, mdbook-katex)`
`[ztl] ecosystem remark: detected (node v20.10.0, harness v1.0.3)`

Metric: `ztl_ecosystem_status{ecosystem, status}` gauge; `status` ∈ {detected, missing, version_incompatible}.

### OBS-3302: Per-Ecosystem Invocation Rate

Metric: `ztl_ecosystem_invocation_total{ecosystem, plugin, outcome}` counter.
Log per build summary: `[ztl] ecosystem pandoc: 3 filters loaded, 247 invocations, 0 failures, 2 marker-strip warnings`.

### OBS-3303: Ecosystem Runtime Resource Usage

Metrics:
- `ztl_ecosystem_subprocess_memory_mib{ecosystem, plugin}` gauge.
- `ztl_ecosystem_subprocess_cpu_ms{ecosystem, plugin}` histogram.
- `ztl_ecosystem_subprocess_restart_total{ecosystem, plugin, reason}` counter (OOM, crash, timeout).

### OBS-3304: Translation Fidelity Warnings

Metric: `ztl_ecosystem_marker_strip_total{ecosystem, plugin, marker_type}` counter.
Log: `[ztl] ecosystem pandoc: pandoc-crossref dropped 4 ztl-wikilink markers on projects/q2-review.md`.

### OBS-3305: Matrix Tier Distribution

`ztl_ecosystem_matrix_plugins_by_tier{ecosystem, tier}` gauge, emitted per build summary.

---

## 10. Security Considerations

- **Ecosystem runtimes are user-installed, user-trusted.** ztl does not bundle Pandoc, Node, or mdBook; it spawns whatever the user's `$PATH` provides. Users who install untrusted binaries get whatever risk those binaries bring; ztl amplifies nothing.
- **Plugin code is untrusted** (same posture as SPEC-032 §10). Each ecosystem has its own isolation characteristics:
  - Pandoc filters: subprocess with the user's UID/GID. No ztl-level isolation beyond what the OS provides. Users wanting sandboxing should run ztl under bwrap/Firejail/Docker themselves.
  - mdBook preprocessors: same.
  - remark plugins: run inside Node's single-threaded event loop; a plugin can access the filesystem and network through Node's APIs. If a user `npm install`s a malicious plugin, ztl inherits the risk.
...
- **Supply chain posture per ecosystem:**
  - Pandoc: user installs via distro package manager; ztl verifies version via `--version` output. CVEs in Pandoc are Pandoc's team problem.
  - mdBook preprocessors: typically Rust binaries from `cargo install`; supply-chain risk is `crates.io`.
  - remark plugins: `npm install`; supply-chain risk is `npm`. ztl does not run `npm audit`; users should.
...
shared safe-mode surface — `ztl build --no-hooks` skips every hook
(ztl-native and ecosystem) and produces plain pipeline output, and
...
`ztl build --no-hooks` first, then re-enable incrementally by
inspecting `ztl theme show`'s declared-hooks list.
...
- **`docs/ecosystems/pandoc.md`** — invocation contract, translation table, matrix excerpts, example manifests, `ztl ecosystem check` walkthrough.
...
- `ztl ecosystem check` subcommand (reports zero ecosystems available initially).
...
1. **SPEC-032 scope narrowing.** ~~SPEC-032 currently ships canonical
   ...
   configured in `.ztl/hooks/`. This removes Python/Node as a
   default-theme runtime dependency, keeps theme-layer design ownership
   in ztl, and delegates transformation ownership to the ecosystem.
2. **Pandoc parser bundling vs subprocess.** ADR-3302 picks subprocess. If users complain about per-page Pandoc invocation cost, should ztl ship pandoc.wasm as an opt-in feature? *Proposed:* defer; measure real-world cost first.
...
4. **Plugin-install orchestration.** Should `ztl ecosystem install pandoc-crossref` be a thing? *Proposed:* no; ztl defers to each ecosystem's native install mechanism and only provides hints.
...
6. **Quarto integration.** Quarto layers on Pandoc; if ztl supports Pandoc well, Quarto-produced filters mostly work. Should we document the Quarto path explicitly? *Proposed:* yes, in `docs/ecosystems/pandoc.md`, as a "Using Quarto filters" subsection.
...
10. **Transient subprocess failures.** Pandoc filter crashes mid-build: does ztl retry? Per SPEC-032 REQ-3207 the page fragment reverts. But if the crash is transient (GC pause, transient OOM), a retry might succeed. *Proposed:* no retry in v1; revisit if field data shows transient failures are common.
