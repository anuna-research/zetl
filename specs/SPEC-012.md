---
title: "SPEC-012: ztl — Named Themes for Serve and Build"
version: 0.2.0
status: draft
...
# SPEC-012: ztl — Named Themes for Serve and Build
...
| Related        | SPEC-009: ztl view — Xanadu-Inspired Transclusion TUI         |

---

## 1. Overview

ztl's `serve` and `build` commands render a vault as a navigable website — `serve` runs a live development server with edit capabilities, and `build` generates a static site for deployment. Both commands currently generate all HTML in Rust code via `format!()` string concatenation in `html.rs`, `routes.rs`, and `build.rs`. CSS comes from CDN links (DaisyUI/Tailwind), JavaScript is inline, and the layout is hardcoded. This means any customisation — loading custom JS like Fountain.js for screenplay rendering, changing the layout, adding analytics, applying a brand stylesheet — requires recompiling ztl.

This specification introduces a **template engine** and **named theme** system that decouples vault data from HTML rendering. ztl adopts a "headless CMS" pattern: structured vault data (pages, links, backlinks, frontmatter, stats) is passed to a Jinja2-compatible template engine, which renders it into HTML. The current UI becomes the built-in **`"default"` theme** — shipped embedded in the binary. Users create additional themes as named subdirectories under `.ztl/themes/` and select the active theme via a `--theme` CLI flag. Each theme only needs to override the templates it wants to change — missing templates fall back to the built-in default.

### 1.1 Core Insight

A static site generator without user-customisable templates is not a static site generator — it's just an HTML exporter. The value of ztl's `serve` and `build` commands is not the specific HTML they produce, but the structured vault data they expose: resolved wikilinks, backlink graphs, transclusion cards, search indices, breadcrumbs. Separating data from presentation unlocks the entire design space: screenplay vaults, documentation sites, digital gardens, research wikis — each with distinct layouts, scripts, and styles — all from the same underlying vault.

### 1.2 Design Philosophy

1. **Data, not markup.** Templates receive structured, serializable context objects. The template decides what to render and how. ztl's job is to provide complete, well-typed data — not HTML fragments.
...
3. **Zero-config default.** With no `.ztl/themes/` directory and no `--theme` flag, the output is pixel-identical to the current hardcoded HTML. The template engine is invisible until the user opts in.
4. **Static assets are first-class.** A `.ztl/static/` directory provides shared assets across all themes, and each theme can bundle its own static assets in a `static/` subdirectory. Assets are served verbatim (in `serve` mode) or copied to the output (in `build` mode) at `/_static/` paths. No build toolchain, no bundler — just files.
...
6. **Themes are named, switchable directories.** Multiple themes can coexist as peer directories under `.ztl/themes/`. Switching themes is a single CLI flag (`--theme fountain`), not a file-shuffling operation. This encourages experimentation and per-audience rendering from a single vault.
...
- Named theme directories under `.ztl/themes/<name>/` with fallback to built-in default
...
- Shared static asset serving from `.ztl/static/` at `/_static/` in serve mode
- Per-theme static assets from `.ztl/themes/<name>/static/` merged with shared assets
...
- Theme packaging or distribution (future SPEC — theme marketplace, `ztl theme install`)
...
Constraints:
  ...
  - Does not want to fork or recompile ztl
  - Wants changes to take effect immediately on `ztl serve`
Daily workflow:
  1. Create `.ztl/themes/garden/base.html` to add analytics script
  2. Run `ztl serve --theme garden` — see changes reflected in the browser
  3. Run `ztl build --theme garden` — static site includes the custom theme
...
Daily workflow:
  ...
  2. Place `fountain.js` in `.ztl/themes/fountain/static/`
  3. Create `.ztl/themes/fountain/page.html` with conditional Fountain loading
  4. Run `ztl serve --theme fountain` — screenplay pages render with Fountain.js
```

### 2.3 Documentation Engineer

```
Role: Technical writer maintaining project documentation as a ztl vault
Goals:
...
Daily workflow:
  1. Create `.ztl/themes/docs-corp/` with full template overrides
  2. Place brand CSS/JS in `.ztl/themes/docs-corp/static/`
  3. Use `ztl build --theme docs-corp -o docs/` to generate static site
...
Goals:
  ...
  - Use `ztl build` output for downstream processing
Constraints:
...
Daily workflow:
  1. Write `.ztl/themes/agent-output/page.html` with structured data access
  2. Run `ztl build --theme agent-output` and verify output programmatically
...
Context:
  ztl's serve and build commands generate HTML via Rust format!()
...
Decision:
  ...
  - Lighter dependency footprint than Tera (~faster compile times).
    ztl's lean dependency set is a design value.
...
Template Loading Order (two-tier):
  ...
  1. Check .ztl/themes/<active-theme>/<name>.html on disk
...
│  │  │                                                         │  │ │
│  │  │  Loader (--theme <name>):                               │  │ │
│  │  │    1. .ztl/themes/<name>/<tpl>.html (disk)             │  │ │
...
Context:
  ...
  A. Single shared .ztl/static/ directory only
  B. Per-theme static/ subdirectory merged with shared .ztl/static/
...
Decision:
  ...
  Resolution order for /_static/foo.js:
    1. .ztl/themes/<active-theme>/static/foo.js
    2. .ztl/static/foo.js
...
Context:
  ztl already strips YAML frontmatter from Markdown content before
...
Decision:
  ...
  - Frontmatter schemas vary per vault and per page. A typed struct
    would require ztl to anticipate all possible frontmatter fields.
...
Consequences:
  ...
  + No schema maintenance burden on ztl
...
FOR all user roles
WITH zero-config default behaviour (no .ztl/themes/ directory or
--theme flag required)
...
.ztl/themes/<theme-name>/ when a theme is selected, falling back
to the built-in default theme for any template not overridden.

The loading order SHALL be:
  a) Check .ztl/themes/<active-theme>/<name>.html on disk
...
FOR all user roles
WITH immediate effect on next `ztl serve` request or `ztl build` run
...
`ztl serve`, with a two-tier resolution order:

  a) .ztl/themes/<active-theme>/static/<path> (theme-specific)
  b) .ztl/static/<path> (shared)
...
`ztl build`, using a two-tier merge:

  a) Copy .ztl/static/ contents first (shared assets)
  b) Copy .ztl/themes/<active-theme>/static/ contents second
...
that selects the active theme.
  ...
  b) When a non-default theme is specified, the system SHALL verify
     that .ztl/themes/<name>/ exists and is a directory; if not,
...
With no .ztl/themes/ directory present and no --theme flag, the output
of `ztl serve` and `ztl build` SHALL be identical to the output
...
able to write ztl templates without learning a new syntax.
```
...
Themes are named directories under .ztl/themes/ relative to the vault
...
Template loading precedence:
  1. .ztl/themes/<active-theme>/<name>.html (theme override)
...
Static assets are served at /_static/ during `ztl serve` with a
two-tier resolution order:

  1. .ztl/themes/<active-theme>/static/<path> (theme-specific)
  2. .ztl/static/<path> (shared)

Per-theme assets take precedence over shared assets on path conflict.

Examples (with --theme fountain):
  .ztl/themes/fountain/static/fountain.js  →  GET /_static/fountain.js
  .ztl/themes/fountain/static/fountain.css →  GET /_static/fountain.css
  .ztl/static/logo.png                     →  GET /_static/logo.png
  .ztl/static/css/main.css                 →  GET /_static/css/main.css

If .ztl/themes/fountain/static/logo.png AND .ztl/static/logo.png
...
During `ztl build`, static assets from both the shared directory and
the active theme's static directory are merged into {out_dir}/_static/,
preserving directory structure.

Copy order:
  1. .ztl/static/* → {out_dir}/_static/ (shared assets first)
  2. .ztl/themes/<active-theme>/static/* → {out_dir}/_static/
     (theme assets second — overwrite shared on conflict)

Examples (with --theme fountain):
  .ztl/static/logo.png                     →  dist/_static/logo.png
  .ztl/static/css/main.css                 →  dist/_static/css/main.css
  .ztl/themes/fountain/static/fountain.js  →  dist/_static/fountain.js
...
The --theme flag is accepted by both `serve` and `build` commands:
  ...
  ztl serve --theme <name>
  ztl build --theme <name>
...
When --theme is "default":
  ...
  - Static assets are served from .ztl/static/ only

When --theme is any other value:
  - The system verifies .ztl/themes/<name>/ exists
  - Templates are loaded from .ztl/themes/<name>/ with fallback
    to built-in defaults
  - Static assets are resolved from .ztl/themes/<name>/static/
    then .ztl/static/
...
Error on invalid theme:
  Error: theme "nonexistent" not found at .ztl/themes/nonexistent/
...
When: ztl build is run with no .ztl/themes/ directory and no --theme flag
Then:
...
When: ztl build is run with default theme (no --theme flag)
Then:
...
Given: A vault with .ztl/themes/test-theme/page.html containing:
       {% extends "base.html" %}
       {% block content %}
       <div class="custom-banner">Custom!</div>
       {{ page.content_html|safe }}
       {% endblock %}
When: ztl serve --theme test-theme is running and a page is requested
Then:
...
Given: A vault with .ztl/themes/minimal/base.html containing a minimal layout
When: ztl build --theme minimal is run
Then:
...
Scenario: No .ztl/themes/ directory and no --theme flag
Given: A vault with no .ztl/ directory or no themes/ subdirectory
When: ztl serve or ztl build is run
Then:
...
Given: A vault with .ztl/themes/alpha/ and .ztl/themes/beta/,
       each containing different page.html templates
When: ztl build --theme alpha is run, then ztl build --theme beta
Then:
...
When: The page is rendered via ztl serve
Then:
  ...
  - ztl.version matches the binary version
...
Given: A vault with .ztl/static/test.js containing "console.log('ok')"
       and .ztl/static/css/style.css containing "body { color: red; }"
When: ztl serve is running (default theme)
Then:
...
Given: A vault with:
       - .ztl/static/style.css containing "body { color: red; }"
       - .ztl/themes/blue/static/style.css containing "body { color: blue; }"
       - .ztl/static/logo.png (shared, not overridden by theme)
When: ztl serve --theme blue is running
Then:
...
Given: A vault with no .ztl/static/ directory and no theme static directory
When: ztl serve is running
Then:
...
Given: A vault with:
       - .ztl/static/common.js
       - .ztl/static/style.css containing "shared"
       - .ztl/themes/custom/static/theme.js
       - .ztl/themes/custom/static/style.css containing "theme"
When: ztl build --theme custom -o dist is run
Then:
...
Given: .ztl/themes/broken/page.html contains {% if unclosed %}
When: A page is requested via ztl serve --theme broken
Then:
...
Given: .ztl/themes/broken/page.html contains {{ undefined_var.deep.access }}
When: ztl build --theme broken is run
Then:
...
Given: A vault with .ztl/themes/fountain/ containing page.html
When: ztl serve --theme fountain is run
Then:
...
Given: A vault with no .ztl/themes/nonexistent/ directory
When: ztl serve --theme nonexistent is run
Then:
...
Given: A vault with no .ztl/ directory
When: ztl serve (no --theme flag) is run
Then:
  - Built-in default theme is used
  - No error about missing .ztl/themes/default/

Scenario: Invalid theme name is rejected
Given: Any vault
When: ztl serve --theme "../escape" is run
Then:
  - The command exits with a non-zero exit code
...
Example output:
  [ztl] active theme: "fountain"
  [ztl] template "page.html" loaded from .ztl/themes/fountain/page.html
  [ztl] template "base.html" using built-in default
  [ztl] template "index.html" using built-in default
  [ztl] template "folder.html" using built-in default
  [ztl] static assets: .ztl/themes/fountain/static/ + .ztl/static/
```
...
**Verification:** `cargo build` succeeds. `ztl serve` and `ztl build` produce identical HTML. Existing tests pass.

### Phase 2: Named themes and `--theme` flag

**Goal:** Support named theme directories under `.ztl/themes/` with a `--theme` CLI flag for theme selection. Templates load from the active theme with fallback to built-in defaults.
...
**Theme name validation:** Reject names containing `/`, `\`, or `..`. Verify `.ztl/themes/<name>/` exists on disk for non-`"default"` themes.

**Verification:** Create `.ztl/themes/test/page.html`, run `ztl serve --theme test`, verify the theme's template renders. Run without `--theme`, verify built-in default is used. Run with `--theme nonexistent`, verify clear error message.

### Phase 3: Static asset serving (per-theme + shared)

**Goal:** Serve user files from `.ztl/static/` (shared) and `.ztl/themes/<name>/static/` (per-theme) at `/_static/`, with theme-specific assets taking precedence.

**Files modified:**
- `src/web/mod.rs` — add `/_static/` route with two-tier static file lookup (theme dir first, then shared dir)
- `src/web/build.rs` — copy `.ztl/static/` then `.ztl/themes/<name>/static/` to `{out_dir}/_static/` after HTML generation
...
1. **Should template hot-reload be supported in serve mode?**
   If a user edits `.ztl/themes/fountain/page.html` while `ztl serve --theme fountain` is running, should the next request pick up the change automatically? This requires either re-creating the Minijinja environment on each request (simple, slight performance cost) or integrating with SPEC-008's file watcher to invalidate the template cache. Recommendation: reload on every request in serve mode (templates are small, parsing is fast); cache in build mode.

2. **Should custom Minijinja filters be supported?**
   Users might want filters like `{{ page.content_raw|wordcount }}` or `{{ page.title|slugify }}`. Minijinja supports registering custom filters. This could be a future SPEC or a built-in set of ztl-specific filters. Recommendation: defer to a future SPEC; the built-in Minijinja filter set (upper, lower, trim, length, join, etc.) is sufficient for the initial release.

3. **Should the default theme be extractable?**
   A `ztl theme export [--theme default]` command could write the built-in templates to `.ztl/themes/<name>/` as a starting point for customisation. This is convenient but could be confusing (user now has a copy that diverges from updates). Recommendation: document that users can copy from the source repository; a dedicated command is a future enhancement.

4. **How should template errors interact with the build process?**
   If one page fails to render, should `ztl build` abort immediately or continue rendering other pages and report errors at the end? Recommendation: fail fast — a partial build is worse than no build, because it might be deployed accidentally.
...
| Feature | Description |
| --- | --- |
| Theme packaging | `ztl theme install <url>` — download and install community themes to `.ztl/themes/` |
| Theme configuration | `theme.toml` metadata file for theme name, author, description, variables |
| Theme listing | `ztl theme list` — enumerate available themes (built-in + user-installed) |
| Template hot-reload | Integrate with SPEC-008 watch mode for live template reloading |
| Custom filters | Register ztl-specific Minijinja filters (wordcount, reading_time, date formatting) |
...
| Theme export | `ztl theme export` to copy built-in templates as customisation starting point |
