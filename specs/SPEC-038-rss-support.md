...
**URL-pool disjointness with SPEC-034 capability tokens:** the
`/caps/<token>/...` URL prefix is shared with SPEC-034 capability-
protected pages by design — both surfaces sit behind the same
auth boundary, so a cohort-token-bearing reader sees the per-page
capability views AND the cohort feed at the same URL shape. The
two token pools, however, do not overlap by construction:

- SPEC-034 capability tokens are minted *per grantee* by
  `zetl cap invite` and recorded in `grants.toml`. They are never
  written into `.zetl/config.toml`.
- SPEC-038 cohort tokens are *operator-set* at config time in
  `[[capability_cohorts]] token` (or generated by `zetl cap cohort
  new`) and never appear in `grants.toml`.

A given URL `/caps/<token>/...` therefore resolves to either a
SPEC-034 capability view (token matches a grant) OR a SPEC-038
cohort surface (token matches a cohort), never both. The
≥128-bit entropy floor in [[#NFR-3809]] makes accidental collision
between the two pools cryptographically negligible. Operators
SHALL NOT manually copy a cohort token into a grant, nor a grant
token into a cohort declaration; the registries are disjoint by
contract.
