...
# SPEC-016: zetl hooks — Git-Style Lifecycle Hooks for Vault Operations
...
zetl is a monolithic binary. Every feature — search, build, check, serve — is compiled in. This works well for the core graph engine, but users inevitably need vault-specific behaviour that doesn't belong in the binary: assembling a screenplay from chained scenes, generating an RSS feed, exporting citations, linting prose style, syncing to a CMS, notifying a Slack channel on save.

This specification adds **lifecycle hooks** — executable scripts that zetl invokes at defined points during its operations. The design follows git's hook model: named scripts in a well-known directory, receiving structured context, controlling flow via exit codes.

### 1.1 Core Insight

zetl already computes rich structured data at every lifecycle point — the link graph, parsed frontmatter, Merkle trees, diagnostics, search index. Today that data is only consumable via CLI JSON output (after the fact) or templates (at render time). Hooks expose the same data **during the operation**, enabling scripts to participate in the pipeline rather than merely react to its output.

### 1.2 Design Philosophy

1. **Git's model, not npm's** — hooks are local, executable files in `.zetl/hooks/`. No package manager, no dependency resolution, no registry. A hook is a script you can read, edit, and understand.
2. **Data in, artifacts out** — hooks receive structured JSON context on stdin and produce side effects (files, stdout, stderr). They do not modify zetl's internal state.
3. **Any language** — hooks are executables. Bash, Python, Deno, a compiled binary — anything with a shebang line (or `.exe` on Windows).
4. **Themes bundle hooks** — a theme's `hooks/` directory contains hooks that activate when the theme is selected. This is how domain-specific build logic (e.g., screenplay assembly) ships with the theme rather than being hardcoded in zetl.
...
- `.zetl/hooks/` directory for vault-level hooks
- Theme-bundled hooks in `.zetl/themes/<name>/hooks/` (or `themes/<name>/hooks/` for bundled themes)
...
- Environment variables for common fields (`ZETL_VAULT_ROOT`, `ZETL_OUT_DIR`, `ZETL_THEME`, etc.)
...
- `zetl hook list` command to show active hooks
- `zetl hook run <name>` command to manually invoke a hook with current vault context
...
- Hooks that modify zetl's in-memory state (hooks are side-effect-only)
...
Goals:
  ...
  - Get assembled .fountain files automatically when running zetl build
...
Daily workflow:
  1. zetl serve --theme fountain → writes scenes, sees prev/next navigation
  2. zetl build --theme fountain → dist/ contains per-scene HTML AND screenplay.fountain
...
Steps:
  1. zetl build --theme fountain
  2. zetl runs build pipeline (scan, render, static assets)
  3. zetl discovers themes/fountain/hooks/post-build
  4. zetl invokes post-build hook with vault context JSON on stdin
...
```
Role: Engineer publishing internal documentation via zetl
Goals:
...
Daily workflow:
  1. Write hooks in .zetl/hooks/ as shell/Python scripts
  2. zetl build → post-build hook generates feed.xml
  3. zetl serve → on-save hook sends Slack notification
  4. zetl check → post-check hook runs prose linter
```

**Happy Path: Write a Custom Post-Build Hook**

```
Preconditions: Vault with .zetl/ directory. jq installed.
Steps:
  1. Create .zetl/hooks/post-build:
     #!/bin/bash
     # Generate RSS feed from pages with "date" frontmatter
     jq -r '.pages[] | select(.frontmatter.date) | ...' < /dev/stdin > "$ZETL_OUT_DIR/feed.xml"

  2. chmod +x .zetl/hooks/post-build
  3. zetl build → hook runs, feed.xml appears in output
  4. zetl hook list → shows "post-build  .zetl/hooks/post-build  (vault)"
...
```
Role: Developer creating a zetl theme that needs custom build output
Goals:
...
Steps:
  ...
  2. zetl build --theme fountain
     → zetl finds themes/fountain/hooks/post-build (bundled theme hook)
     → invokes it with vault context
     → screenplay.fountain written to out_dir

  3. User installs theme via zetl theme install, hooks come along automatically.

Postconditions: Theme hooks run for any user who activates the theme.
Failure modes:
  - Theme installed from git, hook not marked executable → zetl makes it executable at install time (or warns)
```
...
1. **Theme hooks:** `<theme-dir>/hooks/<hook-name>` — from the active theme's directory (disk-installed themes at `.zetl/themes/<name>/hooks/`, bundled themes at compile-time embed)
2. **Vault hooks:** `.zetl/hooks/<hook-name>` — vault-local hooks
...
| --- | --- | --- |
| `pre-build` | Before `zetl build` renders any pages | Yes (non-zero exit skips build) |
| `post-build` | After `zetl build` completes all rendering and asset copying | No (warning only) |
| `post-index` | After `zetl index` completes scanning and indexing | No |
| `post-check` | After `zetl check` collects all diagnostics | No |
| `on-save` | After a page is saved in `zetl serve` and the vault is reindexed | No |
| `pre-serve` | Before `zetl serve` starts the HTTP server | Yes |
...
| --- | --- | --- |
| `ZETL_HOOK` | The hook name being invoked | `post-build` |
| `ZETL_VAULT_ROOT` | Absolute path to the vault root | `/Users/jane/screenplay` |
| `ZETL_THEME` | Active theme name | `fountain` |
| `ZETL_VERSION` | zetl version string | `0.9.0` |

Additional variables for specific hooks:

| Variable | Hooks | Value |
| --- | --- | --- |
| `ZETL_OUT_DIR` | `pre-build`, `post-build` | Absolute path to the build output directory |
| `ZETL_SAVED_FILE` | `on-save` | Relative path of the saved file |
| `ZETL_SAVED_PAGE` | `on-save` | Page name of the saved file |
| `ZETL_PORT` | `pre-serve` | Port number for the server |
...
The timeout is not configurable in this version (future: `.zetl/config.toml` hook timeout setting).
...
The context SHALL include enough data for hooks to perform useful work without needing to re-scan the vault or call zetl subcommands.
...
- **Disk-installed themes:** `.zetl/themes/<name>/hooks/<hook-name>`
- **Bundled themes:** Embedded at compile time from `themes/<name>/hooks/<hook-name>`

For bundled themes, hook files cannot be executed directly from the embedded binary. The system SHALL extract bundled hook files to a temporary directory (or `.zetl/cache/hooks/<theme>/`) before execution. Extracted hooks are refreshed when the zetl binary version changes.
...
Theme hooks installed via `zetl theme install` (SPEC-014) SHALL be made executable (`chmod +x`) during installation. The system SHALL log the hooks being installed:
...
- Explicitly installed themes (`zetl theme install`)
- Vault-local hooks (`.zetl/hooks/`, user-created)
...
REQ-016-010: `zetl hook list`

The system SHALL provide a `zetl hook list` subcommand that lists all active hooks for the current vault and theme. Default output is JSON; `-f table` produces a human-readable table.
  ...
  "hooks": [
    {
      "name": "post-build",
      "source": "theme",
      "path": ".zetl/themes/fountain/hooks/post-build",
      "executable": true
    },
    {
      "name": "on-save",
      "source": "vault",
      "path": ".zetl/hooks/on-save",
      "executable": true
    }
  ]
}
...
```
 ...
 post-build   theme  .zetl/themes/fountain/hooks/post-build    yes
 on-save      vault  .zetl/hooks/on-save                       yes
```
...
REQ-016-011: `zetl hook run <name>`

The system SHALL provide a `zetl hook run <name>` subcommand that manually invokes a hook with the current vault context. This is useful for testing hooks during development.
...
Hooks SHALL NOT be able to modify zetl's in-memory state. They are separate processes with no shared memory. They can only affect the world through their own I/O (writing files, network calls, stdout/stderr).
...
If a hook crashes (segfault, panic) or times out, the parent operation SHALL continue normally (except for pre-hooks, which abort). No hook failure shall corrupt vault data, the link graph, or the build output produced by zetl itself.
...
**Decision:** Executable hooks in `.zetl/hooks/`, invoked as child processes.

**Rationale:**

- **Zero coupling** — hooks are separate processes. They cannot corrupt zetl's memory, deadlock its threads, or introduce undefined behaviour. A crashing hook is just a non-zero exit.
...
- **Version-controllable** — hooks are files in `.zetl/hooks/`, committed alongside the vault. No external state to manage.
- **Debuggable** — `zetl hook run post-build` invokes the hook with real data. Print-debugging works. No plugin framework to understand.
...
- **No zetl API access** — hooks cannot query the search index or traverse the graph incrementally. They receive a snapshot of data. For deep integration, the hook would need to shell out to `zetl search` or parse the JSON context. This is by design — hooks are for side effects, not for extending the core engine.
...
- Lua/JS embedding: ties zetl to a specific runtime, increases binary size, creates a maintenance burden.
...
- **No binary bloat** — screenplay assembly doesn't increase zetl's binary size. It's a script in the theme.
- **Swappable** — users can override a theme's hook by placing their own version in `.zetl/hooks/` with the same name (vault hooks run after theme hooks, and can override side effects).
- **Discoverable** — `zetl hook list` shows what hooks a theme provides. No hidden feature flags.
...
- **Environment variables for convenience** — `ZETL_OUT_DIR`, `ZETL_VAULT_ROOT` etc. are available without parsing JSON, making trivial hooks simpler.
...
```json
{
  ...
  "type": "object",
  "required": ["hook", "vault_root", "theme", "zetl_version", "pages", "stats"],
  "properties": {
    ...
    "zetl_version": { "type": "string", "description": "zetl version" },
    "pages": {
      "type": "array",
      "items": {
        "type": "object",
...
```
Invocation:
  1. Set environment variables (ZETL_HOOK, ZETL_VAULT_ROOT, ZETL_THEME, ZETL_VERSION, ...)
...
Exit codes:
  ...
  126       → hook not executable (zetl detects this before spawn)
...
Scenario: `.zetl/hooks/post-build` exists and is executable.
Expected: Discovered and listed by `zetl hook list`.

Scenario: `.zetl/hooks/post-build` exists but is NOT executable.
Expected: Warning logged. Not invoked.

Scenario: Theme has `hooks/post-build`, vault also has `.zetl/hooks/post-build`.
...
Scenario: `.zetl/hooks/post-build` — recognised lifecycle point.
Expected: Invoked after build.

Scenario: `.zetl/hooks/my-custom-thing` — unrecognised name.
Expected: Silently ignored. No warning.

Scenario: `.zetl/hooks/README.md` — supporting file.
...
Scenario: `post-build` hook reads stdin, writes a file to `$ZETL_OUT_DIR/custom.txt`.
...
Scenario: `post-build` hook prints `$ZETL_VAULT_ROOT`.
Expected: Output matches the vault's absolute path.

Scenario: `post-build` hook prints `$ZETL_OUT_DIR`.
Expected: Output matches the build output directory.

Scenario: `on-save` hook prints `$ZETL_SAVED_FILE`.
...
Expected: Count matches `zetl check` output.
...
Scenario: Active theme has `hooks/post-build`. Run `zetl build --theme <name>`.
...
Scenario: `zetl theme install user/repo` where theme contains `hooks/post-build`.
...
Expected: `zetl hook list` shows both with correct source and path.
...
Scenario: `zetl hook run post-build`.
Expected: Runs the post-build hook with current vault context. Stdout printed.

Scenario: `zetl hook run on-save -- '{"saved":{"file":"test.md","page":"Test","content_length":100}}'`.
Expected: Runs on-save hook with merged context.

Scenario: `zetl hook run nonexistent`.
...
**Changes:**
- Add hook discovery module: scan `.zetl/hooks/` and theme hooks directories
...
- Add `ZETL_*` environment variables
...
- For bundled themes: extract embedded hooks to `.zetl/cache/hooks/<theme>/` before execution
- Update `zetl theme install` to make hook files executable
- Log theme hooks during installation
- Run theme hooks before vault hooks at each lifecycle point

**Verification:** Install a theme with hooks via `zetl theme install`. Verify hooks run on build. Verify bundled theme hooks are extracted and executed. Verify theme + vault hooks both run in correct order.

### Phase 4: Management Commands and Polish

**Goal:** Add `zetl hook list` and `zetl hook run`, polish logging and error messages.

**Changes:**
- Add `Hook` subcommand group to `cli.rs` with `List` and `Run` variants
- Implement `zetl hook list` with JSON and table output
- Implement `zetl hook run <name>` with optional context merging
- Add verbose logging (OBS-016-001, OBS-016-002)
- Polish all warning/error messages

**Verification:** `zetl hook list` shows all hooks with metadata. `zetl hook run post-build` invokes the hook with real data. Verbose output is informative.

---

## 9. Open Questions

1. **Should hooks be able to produce diagnostics that appear in `zetl check` output?**
   A `post-check` hook could print JSON diagnostics to stdout that zetl merges into the check output. This would let hooks act as custom linters whose findings appear alongside native diagnostics. Recommendation: defer — hooks can print their own warnings to stderr for now. Structured diagnostic integration is a future enhancement.
...
| Feature | Description |
| --- | --- |
| Hook configuration | `.zetl/config.toml` settings for hook timeout, enable/disable per hook |
| Structured diagnostics | Hooks emit JSON diagnostics to stdout that merge into `zetl check` output |
| Watch-mode hooks | `on-change` hook triggered by file watcher (SPEC-008 integration) |
| Hook templates | `zetl hook init <name>` scaffolds a hook from a template |
