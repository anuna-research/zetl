...
`ztl check` is the vault-health command. It reports three kinds of problem — dead links, orphan pages, and syntax errors — and optionally a fourth, SPL diagnostics, if you use the reasoning layer. Run it before a big refactor, before a publish, and in CI.
...
The `ztl similar "Zettelkasten Methog"` command (see [[Similar Pages]]) pairs naturally with dead-link review: for each dead link, check whether you meant an existing page.
...
`ztl check --syntax` reports malformed wikilinks, malformed frontmatter, and other parse-level problems. These are usually copy-paste accidents. Fix them on sight.

## `ztl stats` — the summary view
...
You are about to `ztl build` your public notes vault. A three-minute pass before the build:

```bash
ztl index --no-cache            # fresh index
ztl stats                       # eyeball the numbers
ztl check --dead-links          # fix typos, create missing pages
ztl check --orphans             # decide: link, delete, or leave
ztl check --fail-on error       # gate
ztl build
```
