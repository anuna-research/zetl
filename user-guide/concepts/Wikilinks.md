...
A **wikilink** is how you point from one note to another by name. ztl parses five shapes of wikilink from your Markdown. This page is the syntax reference; for workflow advice see [[Linking Pages]].
...
When you write `[[Some Page]]`, ztl does not look up a URL or an ID. It looks for a file named `Some Page.md` in your vault. Specifically:
...
- **Across subfolders**. ztl searches the whole vault tree; you don't write `[[projects/ztl]]`, just `[[ztl]]`.
- **Slugified** for the web output — `Some Page.md` becomes `/page/some-page/` in `ztl serve` and `ztl build`. You never need to think about slugs when writing; that's what the resolver is for.

If two files share a name across folders, the first match wins and the other is reported as ambiguous by `ztl check`. Rename one.

## Dead links are fine (until they're not)

A wikilink to a page that does not yet exist is a **dead link**. ztl does not crash, does not refuse to render, and does not auto-create the file. It records the edge and surfaces it in:
...
This note has five outbound edges, one of which is an embed. If `Evergreen Notes.md` doesn't exist yet, the link stays in place; `ztl check` will nag you politely.

## What wikilinks are not

- **Not standard Markdown.** `[[...]]` is an extension. ztl, Obsidian, Logseq, and a few others understand it; vanilla CommonMark does not. If you need to publish with a tool that doesn't, `ztl build` turns wikilinks into regular `<a href>` links for you.
