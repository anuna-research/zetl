...
3. ztl issues a fresh session and deletes all your existing passkeys.
...
| `m/44'/2'/0'` | Git SSH key | Derived on demand with `ztl derive-ssh-key`. |
...
If you push the vault's git repo to a remote (GitLab, GitHub, Forgejo, codeberg), you want a stable SSH key for the remote. `ztl derive-ssh-key` materialises the ed25519 key from the mnemonic:

```bash
ztl derive-ssh-key \
  --mnemonic "palm tornado ladder oyster casual umbrella \
              desert finger enlist brave coconut strong" \
  --out ~/.ssh/id_ed25519_ztl
```
...
```bash
ztl agent-token --mnemonic "palm tornado ladder …"
```

Use the printed token as `Authorization: Bearer <token>` against the ztl API.
