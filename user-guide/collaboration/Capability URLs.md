...
Formal spec: see `docs/capability-mode.md` in the ztl repo.
...
- `ztl_CAP_SECRET` — the 48-byte content-encryption secret.
- `ztl_CAP_SIGNING_KEY` — the Ed25519 vault-signing private key.

Capture them somewhere safe (a secret store, a password manager, a sealed envelope). You'll need `ztl_CAP_SECRET` on every build and `ztl_CAP_SIGNING_KEY` whenever you rotate the signing key.
...
`--cohort` groups grants that share a content key. Rotate a cohort without touching the others. `--site-url` is the canonical hostname the invite URL points at (also settable via `ztl_CAP_SITE_URL`).
...
## Two-operator handoff: `ztl cap pair`

For higher-stakes vaults, you don't want the inviter to see the reader's private key even briefly. `ztl cap pair` runs a **SPAKE2 pubkey handoff** authenticated by a short spoken phrase:

- **Grantor** (you) runs `ztl cap pair --grantor`. It generates a fresh 4-word BIP39 phrase, prints it, and starts a SPAKE2 session. You read the phrase aloud to the other operator.
- **Grantee** (the other operator) runs `ztl cap pair --grantee --peer <handshake> --phrase "<4 words>" --pubkey <their-pubkey>`. They send their outbound handshake and HMAC tag back to you.
- You paste those into the blocked grantor session. On a matching phrase, SPAKE2 derives the same shared key on both sides and the HMAC verifies; the authenticated pubkey is now yours to use with `ztl cap invite --recipient`.
...
|---------|--------------|
| `ztl cap finalise <grant>` | Mark a grant as operator-confirmed after first use, so the capability binds to a known device (TOFU). |
| `ztl cap check` | Stale-grant and public-safety audit. Catches capabilities drifting past their useful life. |
| `ztl cap sweep` | Bulk-revoke past-expiry grants. |
| `ztl cap audit-diff` | Scan a vault diff for malicious-content patterns (e.g. a hostile theme attempting exfiltration). |
| `ztl cap rotate-signing-key` | Rotate the vault signing key. Rebuild required — every page is re-signed. |
| `ztl cap emergency-shutdown` | Print the operator checklist for pulling a capability site offline in a hurry. Doesn't modify files. |
...
- **Collab server**: two or more people actively editing. Real-time merge. Per-user commits. Needs a box to run `ztl serve --collab` on.

For the full operator spec — threat model, deploy recipes for Nginx/CloudFront, rotation ordering, CSP headers — see `docs/capability-mode.md` in the ztl repo.
