...
`ztl diff` reconstructs the vault graph at a Git ref by re-parsing only changed files, then computes a structural diff showing what changed in the link graph. This is the only ztl command that requires Git.
