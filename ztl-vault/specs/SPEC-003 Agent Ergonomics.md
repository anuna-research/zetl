...
Documents findings from running ztl as an LLM agent would — systematically exercising every command and identifying friction points. See [[Agent Ergonomics]] for the decision record.
...
|-------------|---------|---------|
| REQ-019 | `ztl list` | Page enumeration for agent discovery |
| REQ-020 | `search --path` | Restrict search to subdirectory |
| REQ-021 | `ztl export` | Full graph adjacency list |
...
These fixes made ztl significantly more reliable as an agent building block. The [[JSON by Default]] philosophy, structured errors, and non-zero exit codes all trace back to this spec.
