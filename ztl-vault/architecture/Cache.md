...
ztl uses two-tier incremental caching for both the [[Link Graph]] and the [[Reasoning Engine]]'s theory.
...
For the theory cache, ztl uses the SPL AST hash (not content hash) to determine if re-reasoning is needed. This means:
...
Both arguments have merit. Run `ztl reason conflicts` on this vault to see how ztl surfaces this kind of tension.
