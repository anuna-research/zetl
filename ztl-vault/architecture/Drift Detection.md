...
ztl uses the [[Merkle Tree]] to track grounding freshness:
...
4. On subsequent `ztl check --drift`, current grounding hashes are compared against cached values
...
```bash
ztl check --drift                       # report drift warnings
ztl check --drift --fail-on warning     # CI: fail on drift
```
