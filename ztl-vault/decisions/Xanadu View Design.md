...
ztl needed a focused reading mode that shows connections between notes. The design question was how to render Ted Nelson's Xanadu concept of "parallel pages with visible connections" in a terminal.
...
The TUI framework was already in use for `ztl tui`. Reusing it for `ztl view` provides consistent behavior across terminal emulators and avoids a second rendering dependency.
