...
A page may contain multiple wikilinks to the same target (e.g., `[[Cache]]` mentioned three times in a single page). Should `ztl links` and `ztl backlinks` report the target once or three times?
...
- Users who need link counts can use `ztl stats` or `ztl export` for the full data
