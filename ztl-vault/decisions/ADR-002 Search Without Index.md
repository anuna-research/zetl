...
ztl needs full-text content search across vault files (see [[Search Command]]). The question is whether to build an inverted index (like Lucene/tantivy) or scan files directly on each query.
...
- **Consistency** — ztl already has an mtime-based cache for the link graph; adding a second indexing system increases complexity disproportionately
