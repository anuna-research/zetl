...
`ztl search` provides full-text content search across all Markdown files in the vault.
...
```bash
# Literal text search
ztl -d . search "defeasible"

# Regex search
ztl -d . search "reason(ing|ed)" --regex

# With surrounding context
ztl -d . search "Cache" --context 100

# Case-sensitive
ztl -d . search "SPL" --case-sensitive

# Restrict to a subdirectory
ztl -d . search "given" --path "theories/*"
```
...
ztl uses index-free scanning rather than an inverted index. This trades query speed on very large vaults for zero build-time indexing cost and simpler architecture. See [[ADR-002 Search Without Index]] for the rationale.
