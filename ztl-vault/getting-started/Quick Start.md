...
A five-step tour of ztl's core capabilities. Assumes you've already run [[Install]].
...
This scans all Markdown files, extracts [[concepts/Wikilinks]] and [[concepts/Spindle Lisp]] blocks, and builds the link graph. The index is cached in `.ztl/` for fast subsequent runs — see [[architecture/Cache]].
...
```bash
ztl -d ./demo-vault check --format table
```
...
```bash
ztl -d ./demo-vault reason status --format table
