...
fn seed_pandoc_lua_filter(stage_dir: &Path, name: &str) -> Result<()> {
    ...
    let body = r#"-- Identity Pandoc Lua filter scaffolded by `zetl hook new --ecosystem pandoc`.
...
/// replace them; they parse cleanly and `zetl ecosystem check` reports
        ...
        HookEcosystem::Mdbook => {
            ...
            format!(
                "\n# Ecosystem adapter: mdbook (SPEC-033 REQ-3312).\n\
                 ...
                 # installed one) — `zetl ecosystem check` reports reachability.\n",
            )
        }
