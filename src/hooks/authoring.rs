...
//!
//! - [`scaffold`] — `ztl hook new <stage> <name>`: writes a persistent-mode
...
//! - [`run_test`] — `ztl hook test <name> [--update]`: runs the hook
//!   against its fixture and diffs the output; `--update` regenerates
//!   the golden instead.
//! - [`capture_fixture`] — `ztl hook fixture --from <page> --hook <name>`:
//!   copies a vault page into the fixture directory and seeds the golden
//!   from the hook's current output.
//! - [`watch`] — `ztl hook watch <name>`: file-watches the hook's source
...
pub fn scaffold(
    ...
    // Composition reads `<filename-with-ext>.toml` (e.g. `foo.py.toml`),
    // not `<stem>.toml`. Match that convention so the scaffolded hook is
    // visible to `compose_stage` without a manual rename.
    let manifest_file = stage_dir.join(format!("{}.{}.toml", name, lang.ext()));
    ...
    if matches!(ecosystem, Some(HookEcosystem::Pandoc)) {
        seed_pandoc_lua_filter(&stage_dir, name)?;
    }
...
/// Drop an identity Lua filter at the path the scaffolded pandoc manifest
/// points at, so `lua_filter = "filters/<name>.lua"` resolves on disk
/// without the author having to write it before `dry-run` or `build`.
/// Skipped if a filter file already exists — never overwrite author work.
fn seed_pandoc_lua_filter(stage_dir: &Path, name: &str) -> Result<()> {
    let filters_dir = stage_dir.join("filters");
    std::fs::create_dir_all(&filters_dir)
        .with_context(|| format!("creating pandoc filters dir {}", filters_dir.display()))?;
    let filter_path = filters_dir.join(format!("{name}.lua"));
    if filter_path.exists() {
        return Ok(());
    }
    let _ = name; // referenced via filename, not body.
    let body = r#"-- Identity Pandoc Lua filter scaffolded by `ztl hook new --ecosystem pandoc`.
-- Replace the function bodies with your transformations; see
--   https://pandoc.org/lua-filters.html
-- for the full element catalogue.
function Pandoc(doc)
  return doc
end
"#;
    std::fs::write(&filter_path, body)
        .with_context(|| format!("writing pandoc lua filter {}", filter_path.display()))?;
    Ok(())
}
...
`ztl hook test <name>` (or `ztl hook test <name> --update` to
...
fn render_manifest(
    ...
    let ecosystem_block = match ecosystem {
        Some(e) => render_ecosystem_block(e, stage, name),
        None => String::new(),
    };
    format!(
        r#"# Manifest for hook `{name}` ({lang_ext}). Generated by `ztl hook new`.
...
/// Seed the per-ecosystem manifest fields required by SPEC-033 REQ-3312
/// so the freshly scaffolded manifest parses without further hand-editing.
/// The chosen placeholders are intentionally obvious so authors know to
/// replace them; they parse cleanly and `ztl ecosystem check` reports
/// whether the named binary / package exists on disk.
fn render_ecosystem_block(eco: &HookEcosystem, _stage: &AuthoringStage, name: &str) -> String {
    // Authors often name their hook with the ecosystem prefix already
    // (e.g. `pandoc-smallcaps`); don't double it in the convention hint.
    let strip_prefix = |prefix: &str| -> &str { name.strip_prefix(prefix).unwrap_or(name) };
    match eco {
        HookEcosystem::Pandoc => {
            let bare = strip_prefix("pandoc-");
            format!(
                "\n# Ecosystem adapter: pandoc (SPEC-033 REQ-3312).\n\
                 # Pandoc filter scaffolded as a Lua filter at the path below — edit\n\
                 # filters/{name}.lua to implement the transformation, or replace\n\
                 # `lua_filter` with `exec = \"pandoc-{bare}\"` to call a native filter.\n\
                 ecosystem = \"pandoc\"\n\
                 lua_filter = \"filters/{name}.lua\"\n",
            )
        }
        HookEcosystem::Mdbook => {
            let bare = strip_prefix("mdbook-");
            format!(
                "\n# Ecosystem adapter: mdbook (SPEC-033 REQ-3312).\n\
                 # `exec` names the mdBook preprocessor binary. Default `scope`\n\
                 # is page; switch to \"vault\" for whole-vault preprocessors.\n\
                 ecosystem = \"mdbook\"\n\
                 exec = \"mdbook-{bare}\"\n\
                 scope = \"page\"\n\
                 # Place the preprocessor binary on PATH (or rename to match an\n\
                 # installed one) — `ztl ecosystem check` reports reachability.\n",
            )
        }
        HookEcosystem::Remark => {
            let bare = strip_prefix("remark-");
            format!(
                "\n# Ecosystem adapter: remark (SPEC-033 REQ-3312).\n\
                 # `package` is resolved by the remark harness via dynamic import().\n\
                 # Pin `version` once you know the working range; remove for `latest`.\n\
                 ecosystem = \"remark\"\n\
                 package = \"remark-{bare}\"\n\
                 # version = \">=1 <2\"\n\
                 # options = {{ }}\n",
...
#   https://ztl.codeberg.page/docs/hook-authoring
{eco_comment}
import json, sys
...
for line in sys.stdin:
    ...
    if t == "probe":
        ...
        # types this hook supports so ztl can self-disable cheaply.
        sys.stdout.write(json.dumps({{
            "type": "probe_result",
            "ztl_ast": "1.0",
            "hook": "{name}",
...
sys.stdout.write(json.dumps({{
    "ztl_ast": 1, "hook": "{name}", "version": "0.1.0", "ready": True,
}}) + "\n")
sys.stdout.flush()
...
fn make_executable(_path: &Path) -> Result<()> {
    // Windows: no-op. `ztl hook test` and `watch` use the file path as
...
fn render_markdown_to_html(content: &str) -> String {
    ...
    // `ztl ast sample --stage post-render`. Matching behaviour avoids
...
/// A hook discovered inside a vault's `.ztl/hooks/<stage>.d/` tree, plus
...
/// Sidecar manifest path for a hook file. Composition canonicalises on
/// `<filename-with-ext>.toml` (e.g. `foo.py.toml`); we also accept the
/// legacy `<stem>.toml` form for hooks scaffolded before that fix
/// landed, so existing trees keep working with `hook test`/`watch`.
fn sidecar_manifest_path(hook_path: &Path) -> Option<PathBuf> {
    let mut buf = hook_path.as_os_str().to_owned();
    buf.push(".toml");
    let canonical = PathBuf::from(buf);
    if canonical.is_file() {
        return Some(canonical);
    }
    let legacy = hook_path.with_extension("toml");
...
pub fn find_scaffolded_hook(vault_root: &Path, name: &str) -> Result<ScaffoldedHook> {
    for stage in Stage::all() {
        ...
        for entry in entries {
            ...
            let manifest_path = sidecar_manifest_path(&path);
    ...
    Err(anyhow!(
        "no hook named '{name}' under .ztl/hooks/{{pre-parse,transform,post-render}}.d/",
    ))
}

// ── Hook test runner (`ztl hook test`) ───────────────────────────────────
...
pub fn run_test(vault_root: &Path, name: &str, update: bool) -> Result<TestOutcome> {
    ...
    if !fixture_dir.is_dir() {
        bail!(
            "fixture directory missing: {} (run `ztl hook new` or \
...
// ── Fixture capture (`ztl hook fixture`) ─────────────────────────────────
...
/// the hook's current output. SPEC-032 REQ-3225 `ztl hook fixture`.
        ...
        match event {
            Ok(Ok(ev)) => {
                ...
                child = match spawn_watched_hook(&hook) {
                    Ok(c) => c,
                    Err(e) => {
                        on_event(&WatchEvent::Stderr(format!("[ztl] respawn failed: {e}\n")));
            ...
            Ok(Err(e)) => {
                on_event(&WatchEvent::Stderr(format!("[ztl] watcher error: {e}\n")));
            }
