...
/// operator dropping into `<out_dir>/_ztl/deploy/` can self-serve.
pub fn render_readme(spec: &HeaderSpec) -> String {
    let mut out = String::new();
    out.push_str("# ztl capability-mode deploy recipes\n\n");
    out.push_str(
    ...
    out.push_str(
        "The `/c/*` max-age is operator-tunable via `[access.cache] max_age` \
         (bounds [60, 3600]). Lowering it shortens the window between `ztl cap \
         revoke` and readers no longer being able to load the page from CDN \
         caches. See NFR-3409.\n\n",
    );
    out.push_str("## CSP fallback\n\n");
    out.push_str(
        "The HTML shell emitted under `_ztl/capability-shell.html` carries the \
...
# ztl capability-mode HTTP header recipe
# Generated by `ztl build` from SPEC-034 REQ-3407 / REQ-3418 / REQ-3421 / REQ-3428.
# Do not edit by hand — re-run the build if you want different values.
";

/// Write all recipes into `<out_dir>/_ztl/deploy/`. Creates missing
...
pub fn write_deploy_recipes(out_dir: &Path, spec: &HeaderSpec) -> Result<PathBuf, io::Error> {
    let deploy_dir = out_dir.join("_ztl").join("deploy");
...
mod tests {
    ...
    fn caddy_recipe_contains_every_rule() {
        ...
        assert!(rendered.contains("@ztl_cap path /c/*"));
        assert!(rendered
            .contains("header @ztl_cap Cache-Control \"private, max-age=300, must-revalidate\""));
        assert!(rendered.contains("@ztl_shim path /assets/shim.js"));
        assert!(rendered.contains("public, max-age=31536000, immutable"));
        for path in CLEAR_SITE_DATA_PATHS {
            assert!(
        ...
        assert!(
            ...
            "expected CSP on @ztl_cap in Caddy recipe, got:\n{rendered}"
        );
        assert!(
