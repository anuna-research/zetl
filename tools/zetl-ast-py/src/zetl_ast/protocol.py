...
zetl speaks line-delimited JSON: one message per line on stdin; the hook
...
def parse_host_message(line: str) -> Dict[str, Any]:
    ...
    if t == "init":
        ...
        return {
            ...
            "zetl_version": str(value.get("zetl_version", "")),
...
def check_ast_major(schema_version: str) -> None:
    ...
    The empty / absent case is tolerated — older zetl builds sent init
    ...
    if major != AST_MAJOR:
        raise ProtocolError(
            f"zetl advertised ast_schema_version={schema_version}; "
