...
Spawns a real Python process that uses ``zetl_ast.run`` as its entry
point and drives it through a full handshake → init → run → finalise →
shutdown cycle from the outside — exactly the shape
``src/hooks/persistent.rs`` drives it with on the zetl side.
...
def _src_path() -> str:
    # ``tests/test_run_integration.py`` → ``tools/zetl-ast-py/src``
...
class TestRunSubprocess(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(os.environ.get("TMPDIR", "/tmp")) / "zetl-ast-py-run-it"
    ...
    def test_full_lifecycle(self) -> None:
        ...
        try:
            ...
            self.assertEqual(handshake["zetl_ast"], 1)
