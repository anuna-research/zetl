...
export interface RunOptions {
    /** Hook id for the handshake — defaults to `$ZETL_EXTENSION_ID` or `"zetl-hook"`. */
    hookId?: string;
 ...
 * zetl's REQ-3207 policy decides whether to continue.
 */
export async function run<In = DocumentNode, Out = In>(
    ...
    const hookId = options.hookId ?? process.env["ZETL_EXTENSION_ID"] ?? "zetl-hook";
    ...
    try {
        ...
        let stage = process.env["ZETL_STAGE"] ?? "transform";
        ...
        (options.stderr ?? process.stderr).write(`zetl-ast-js fatal: ${(e as Error).message}\n`);
...
export async function runOneShot<In = DocumentNode, Out = In>(
    ...
    const { ctx, writes } = buildContext({
        pageSlug: process.env["ZETL_PAGE_SLUG"] ?? "",
        frontmatter: {},
        stage: process.env["ZETL_STAGE"] ?? "transform",
        env: { ...env, extension_id: hookId },
        buildData: undefined,
    });
...
function checkAstMajor(schemaVersion: string): void {
    ...
    if (major !== AST_MAJOR) {
        throw new ProtocolError(
            `zetl advertised ast_schema_version=${schemaVersion}; this hook speaks major ${AST_MAJOR} (pinned to ${AST_VERSION}).`,
            "ast_major_mismatch",
        );
    }
}
...
function defaultEnvFromProcess(hookId: string): BuildEnv {
    return {
        mode: (process.env["ZETL_MODE"] as "build" | "serve" | undefined) ?? "build",
        theme: process.env["ZETL_THEME"] ?? null,
        vault_root: process.env["ZETL_VAULT_ROOT"] ?? "",
        out_dir: process.env["ZETL_OUT_DIR"] ?? null,
        verbose: process.env["ZETL_VERBOSE"] === "true",
        at: process.env["ZETL_AT"] ?? null,
        hook_path: process.env["ZETL_HOOK_PATH"] ?? null,
        extension_id: process.env["ZETL_EXTENSION_ID"] ?? hookId,
    };
}
