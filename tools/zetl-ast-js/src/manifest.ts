...
//     document so tooling (e.g. `zetl hook new`) can scaffold sidecars
//     without pulling in a full TOML serializer;
//   - provides `parseManifest()` that reads the slice zetl's
...
// the shape the zetl composition layer accepts. For arbitrary TOML,
// hook authors should use `@iarna/toml` or similar in their own
// tooling; zetl only consumes what's documented here.

/** AST ecosystem declarations recognised at the composition boundary. */
export type AstType = "zetl-ext" | "mdast-ext" | "pandoc-ext";
...
export interface Manifest {
    ...
    /** Top-level aliases accepted by zetl; merged with `ordering.*` at parse time. */
    before?: string[];
    after?: string[];
    /** Required by transform-stage hooks targeting non-zetl ecosystems (REQ-3221). */
    ast_type?: AstType;
...
const VALID_AST_TYPES: ReadonlySet<string> = new Set(["zetl-ext", "mdast-ext", "pandoc-ext"]);
