...
helpers accumulate writes into a response payload that :mod:`ztl_ast.run`
flushes back to ztl as part of the ``result`` message.
...
class BuildEnv:
    ...
    extension_id: str = "ztl-hook"
    ztl_version: Optional[str] = None
...
def writes_to_result_payload(
    ...
    Mirrors ``ztl-ast-js``'s ``resultFromWrites``: empty sections are
