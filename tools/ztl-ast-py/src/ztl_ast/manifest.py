...
* ``render_manifest()`` to emit a minimal canonical TOML document so
  tooling (e.g. ``ztl hook new``) can scaffold sidecars without pulling
  in a full TOML serialiser.
* ``parse_manifest()`` that reads the slice ztl's composition layer
...
ztl reads. For arbitrary TOML, hook authors should use the stdlib
``tomllib`` (3.11+) or ``tomli`` in their own tooling; ztl only consumes
...
AST_TYPES = ("ztl-ext", "mdast-ext", "pandoc-ext")
