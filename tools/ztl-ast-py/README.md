...
First-party helper library for authoring [ztl](https://ztl.dev) hooks in
Python. Pinned to AST schema **major v1** (SPEC-032 REQ-3210).

## Install

```sh
pip install ztl-ast-py
```
...
```python
from ztl_ast import run, dispatch, on_node, Wikilink
...
page ztl sends — replying with a single `result` / `error` line per
call. On any exception, the loop emits a typed `error` frame; ztl's
...
ztl-ext v1.x. A ztl binary advertising a different AST major in the
