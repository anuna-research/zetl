...
describe("protocol", () => {
    it("handshakeLine embeds the pinned AST major", () => {
        ...
        const parsed = JSON.parse(line) as { ztl_ast: number; hook: string; version: string; ready: boolean };
        assert.strictEqual(parsed.ztl_ast, AST_MAJOR);
    ...
    it("parseHostMessage handles each host message type", () => {
        const init = parseHostMessage(
            JSON.stringify({ type: "init", stage: "transform", ztl_version: "0", ast_schema_version: "1.0", ctx: {} }),
        );
...
describe("run()", () => {
    it("handshake → init → run → finalise → shutdown round-trip", async () => {
        const ast = sampleAst();
        const hostMessages = [
            JSON.stringify({ type: "init", stage: "transform", ztl_version: "0", ast_schema_version: AST_VERSION, ctx: { theme: "default" } }),
    ...
    it("emits {type:error} on ast_version major mismatch during init", async () => {
        const hostMessages =
            JSON.stringify({ type: "init", stage: "transform", ztl_version: "0", ast_schema_version: "2.0", ctx: {} }) +
            "\n";
    ...
    it("emits {type:error} when the transform throws", async () => {
        const ast = sampleAst();
        const hostMessages =
            JSON.stringify({ type: "init", stage: "transform", ztl_version: "0", ast_schema_version: AST_VERSION, ctx: {} }) +
            "\n" +
