...
//
// ztl speaks line-delimited JSON: one message per line on stdin; the
...
export function handshakeLine(hook: string, version: string): string {
    const h: HookHandshake = { ztl_ast: AST_MAJOR, hook, version, ready: true };
...
export type HostMessage =
    | { type: "init"; stage: string; ztl_version: string; ast_schema_version: string; ctx: InitContext }
    ...
    switch (msg.type) {
        case "init": {
            ...
            return {
                ...
                ztl_version: String(msg.ztl_version ?? ""),
