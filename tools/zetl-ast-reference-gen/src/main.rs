//! Generate `docs/zetl-ast-reference.md` from `tools/zetl-ast-schema-v1.json`.
//!
...
//!
//! Usage:
//!   cargo run -p zetl-ast-reference-gen            # write docs/zetl-ast-reference.md
//!   cargo run -p zetl-ast-reference-gen -- --check # exit nonzero on diff
...
const OUT_PATH: &str = "docs/zetl-ast-reference.md";
...
fn main() -> Result<()> {
    ...
    if check_only {
        ...
        if on_disk != rendered {
            eprintln!(
                "zetl-ast-reference-gen: {} is stale; re-run without --check to regenerate.",
                out_path.display()
            );
            std::process::exit(1);
        }
        println!(
            "zetl-ast-reference-gen: {} is up to date.",
            out_path.display()
        );
    } else {
        ...
        println!("zetl-ast-reference-gen: wrote {}", out_path.display());
    }
    Ok(())
}

/// Walk upward from the executable's cwd until we hit the directory that
/// contains both `Cargo.toml` and `tools/zetl-ast-schema-v1.json`. Cargo
...
fn find_repo_root() -> Result<PathBuf> {
    ...
    loop {
        if cur.join("tools/zetl-ast-schema-v1.json").is_file() && cur.join("Cargo.toml").is_file() {
            return Ok(cur.to_path_buf());
        }
        match cur.parent() {
            Some(p) => cur = p,
            None => bail!(
                "could not locate repo root (looking for tools/zetl-ast-schema-v1.json) starting from {}",
                start.display()
            ),
        }
    }
...
fn render(schema: &Value) -> Result<String> {
    ...
    out.push_str("<!--\n");
    out.push_str("  AUTO-GENERATED by tools/zetl-ast-reference-gen from\n");
    out.push_str("  tools/zetl-ast-schema-v1.json. Do not hand-edit; regenerate with:\n");
    out.push_str("      cargo run -p zetl-ast-reference-gen\n");
...
fn html_expectation(name: &str) -> &'static str {
    match name {
        ...
        "Wikilink" => "Renders as `<a href=\"{resolved}\" class=\"zetl-wikilink\">{label}</a>`. `{resolved}` is the slug for `target` (with `#heading` or `#^block-id` appended when set). `{label}` is `alias` when set, otherwise `target`. Unresolvable wikilinks render as `<a href=\"#\" class=\"zetl-wikilink zetl-wikilink-missing\">{label}</a>`.",
