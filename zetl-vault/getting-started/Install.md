...
zetl requires a Rust toolchain. Install one via [rustup](https://rustup.rs/) if you don't already have it.
...
The `history` feature gate pulls in jj-lib for automatic temporal snapshots stored in `.zetl/jj/`. Enables the `--at` flag, `zetl history` commands, and history data in templates, hooks, and API endpoints. See [[History Command]].
