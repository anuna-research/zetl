...
`zetl history` provides commands for browsing vault history. Requires `--features history` at build time. See [[Feature Gates]].

History is powered by jj-lib, which silently initializes a VCS repository at `.zetl/jj/` on first use. Snapshots are created automatically during `zetl index` and `zetl watch`.
...
```bash
zetl history page "Scanner"
zetl history page "Cache" --limit 10
```
...
```bash
zetl history timeline
zetl history timeline --limit 50
```
...
```bash
zetl --at "3 days ago" links "Scanner"
zetl --at "2024-01-15" stats
zetl --at "last monday" check
zetl --at "HEAD~3" backlinks "Cache"
```
...
When running `zetl serve` with history enabled, the following API endpoints are available:
...
`zetl build` writes `history-index.json` to the output directory containing vault-level trend data and per-page history summaries. This can be consumed by client-side JavaScript for history visualizations.
