...
When built with `--features history`, each re-index cycle automatically creates a jj snapshot (deduplicated by vault root hash). This builds a continuous history timeline that can be queried with `zetl history log` or the `--at` flag. See [[History Command]].
