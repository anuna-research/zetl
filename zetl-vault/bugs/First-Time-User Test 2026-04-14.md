...
binary: target/release/zetl (features reason,history,mcp)
vault: /tmp/zetl-firsttime (fresh, 10 markdown pages, 18 links, 3 dead, 1 orphan)
---
...
Verification (post-fix, on the same `/tmp/zetl-firsttime` vault):

```
BUG-001: $ zetl --no-cache index && zetl --json stats | jq '.spl_blocks,.grounded_spl_blocks'
         0
         0                                                                   # invariant holds
BUG-002: $ curl -o /dev/null -w "%{http_code}\n" http://127.0.0.1:.../NonExistent
         404                                                                 # was 200
BUG-005: $ zetl build --out /tmp/zbo   → succeeds
         $ zetl build -o   /tmp/zbo2   → succeeds
```
...
Exercised every top-level `zetl` subcommand on a freshly-created vault containing
...
- zetl 0.1.0, release build with `--features "reason,history,mcp"`
- Vault: `/tmp/zetl-firsttime` with no current SPL content (one SPL block was
...
2. Add a file `Rules.md` with a fenced ` ```spl ` block; run `zetl index`.
3. Delete `Rules.md` from disk.
4. Run `zetl --no-cache index` (forces full rescan).
5. Run `zetl --json stats`.
...
```
$ zetl --no-cache index >/dev/null && zetl --json stats | jq '.spl_blocks,.grounded_spl_blocks'
0
1
```
...
- Violates: HTTP semantics (RFC 9110 §15.5.5 — "404 Not Found"). No zetl REQ
  found addressing the HTTP status code policy for unknown slugs —
  **specification gap**.

### Steps to Reproduce

1. Run `zetl serve --port 18231`.
...
an HTML page titled `stats — zetl` — there is no JSON API at that path, but the
...
<html lang="en" data-theme="default">
  <title>stats — zetl</title>
  <meta name="description" content="stats — a page in the zetl-firsttime knowledge vault.">
```
...
## BUG-003: `zetl diff --from <ref>` silently ignored in a non-git vault
...
- Violates: the `--help` contract for `zetl diff`. `--from` is described as
...
1. Create a vault in `/tmp/zetl-firsttime` with **no** `.git` directory.
2. Run `zetl index` twice with changes in between so a jj snapshot exists.
3. Run `zetl diff` and `zetl diff --from HEAD~1`.

### Expected Behaviour

`zetl diff --from HEAD~1` should either:
...
`zetl diff` and `zetl diff --from HEAD~1` return **byte-identical output**. The
...
1. `rm -rf .zetl` in an empty directory so the semantic model cache is absent.
2. Run `zetl index` with stdin attached to a TTY (or pipe `N\n`).
...
Piping `zetl index | jq .` on a first run produces a JSON parse error. A user
who automates zetl in CI will hit this on any fresh workspace.
...
## BUG-005: `zetl build` uses `--out-dir`; most tools use `--out` / `-o`
...
- Violates: convention, not specification. `zetl serve` accepts `--port`
...
ergonomics. Confirmed low-risk: there is no other `-o` flag on `zetl build`.
...
### Observation A: `zetl check` exit code contract is undocumented

`zetl check` exits **non-zero** when any dead link or orphan is found. This is
the right default for CI use, but:

- It is not mentioned in `zetl check --help`.
- A user running `zetl check && zetl search foo` in a shell will find the
...
### Observation C: `zetl history` uses `timeline`; other multi-verb commands use `list`

`zetl theme list`, `zetl hook list`, `zetl list` all use `list` as the
inventory verb. `zetl history timeline` breaks the pattern. Either add
`history list` as an alias or standardise on the more-expressive `timeline`
everywhere.

### Observation D: `zetl invite --as alice` fails with `"user 'alice' not found in this vault"` on a vault that has never had users configured

The error is correct but the new user has no signpost to "how do I add a user
to this vault?". Propose: include a hint in the error (`"Run 'zetl user add
...
On an empty vault, the first `zetl index` prompts for a `~90 MB` model
download. The prompt text is clear, but this happens before any content has
been indexed — a user trying zetl for the first time with `mkdir demo && cd
demo && zetl index` may reasonably ask "why is a wikilink tool downloading
100 MB?" Consider deferring the prompt until the user first runs a command
that actually needs semantic search, or making it a one-line `zetl setup
...
| BUG-005 | S4       | P3       | `zetl build` lacks `--out` / `-o` alias                 |
...
BUG-004 is borderline release-blocking for anyone piping `zetl index` in CI
...
- **Session context:** Interactive Claude Code session, 2026-04-14. Test
  vault preserved at `/tmp/zetl-firsttime` for reproduction.
